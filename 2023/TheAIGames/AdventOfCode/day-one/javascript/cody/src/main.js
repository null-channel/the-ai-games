...
1925
`;
function findSum2020ExpenseReport2(expenseReport) {
    for (let i = 0; i < expenseReport.length; ++i) {
        for (let j = i + 1; j < expenseReport.length; ++j) {
    ...
    throw new Error("No triplet found");
}

const expenses = expenseReport.split('\n').map(Number);
const solution = findSum2020ExpenseReport(expenses);
const solution2 = findSum2020ExpenseReport2(expenses);
console.log(solution); // 514579
console.log(solution2); // 514579